...
We used the mean absolute error (MAE) to evaluate the performance of proposed prediction models. We train regression models using the combined data set of N (N = 1...number of liquid - 1) known liquid and 2 levels (full and empty levels) of unknown liquid data. To examine how a regression model's performance change when the number of known liquid used for training increases, we gradually increase the number of liquid used from 1 to 8 (9 is the total number of liquid we have). If we use N number of liquid for training, then we will test the model with each one of the rest \[9-N\] liquid data. Therefore, the number of rounds of evaluations for the model trained with N number of liquid data can be expressed as the following equation: \[C^9_N * (9-N)\], \[ N = 1...8\]. For example, the number of rounds of evaluations for the model train with 5 types of liquid data is \[C^9_5 * (9-5) = 630\].     
